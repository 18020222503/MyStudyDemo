...
template<class Value,
    class HashFcn = SPARSEHASH_HASH<Value>,
    class EqualKey = std::equal_to<Value>,
    class Alloc = GOOGLE_NAMESPACE::libc_allocator_with_realloc<Value > >
class Il2CppNotDefaultKeyHashSet : public GOOGLE_NAMESPACE::sparse_hash_set<Value, HashFcn, EqualKey, Alloc>
{
private:
    typedef GOOGLE_NAMESPACE::sparse_hash_set<Value, HashFcn, EqualKey, Alloc> Base;
...
public:
    ...
    explicit Il2CppNotDefaultKeyHashSet(size_type n = 0,
        const hasher& hf = hasher(),
        const EqualKey& eql = EqualKey()) :
        Base(n, hf, key_equal(eql))
    {
        Base::set_deleted_key(Value{});
    }

    template<class InputIterator>
    Il2CppNotDefaultKeyHashSet(InputIterator f, InputIterator l,
        size_type n = 0,
        const hasher& hf = hasher(),
        const EqualKey& eql = EqualKey()) :
        Base(f, l, n, hf, key_equal(eql))
...
template<class Value,
    ...
    class EqualKey = std::equal_to<Value>,
    class Alloc = GOOGLE_NAMESPACE::libc_allocator_with_realloc<Value > >
class Il2CppReaderWriterLockedNotDefaultKeyHashSet
{
public:
    typedef typename Il2CppNotDefaultKeyHashSet<Value, HashFcn, EqualKey, Alloc>::key_type key_type;
    typedef typename Il2CppNotDefaultKeyHashSet<Value, HashFcn, EqualKey, Alloc>::size_type size_type;
    typedef typename Il2CppNotDefaultKeyHashSet<Value, HashFcn, EqualKey, Alloc>::const_iterator const_iterator;
    typedef typename Il2CppNotDefaultKeyHashSet<Value, HashFcn, EqualKey, Alloc>::iterator iterator;
    typedef typename Il2CppNotDefaultKeyHashSet<Value, HashFcn, EqualKey, Alloc>::hasher hasher;

    explicit Il2CppReaderWriterLockedNotDefaultKeyHashSet
    (size_type n = 0,
        const hasher& hf = hasher(),
        const EqualKey& eql = EqualKey()) :
        hashSet(n, hf, eql)
...
private:
    il2cpp::os::FastReaderReaderWriterLock lock;
    Il2CppNotDefaultKeyHashSet<Value, HashFcn, EqualKey, Alloc> hashSet;
};